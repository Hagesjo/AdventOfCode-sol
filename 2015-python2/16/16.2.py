...
# Default dict can be used instead, so all the operator.eq-rows can go away.
ops = {	'children' : operator.eq,
		'cats' : operator.gt,
...
with open('input') as f:
	for line in f:
		...
		for k in candidate.__dict__:
			if candidate.__dict__[k] is None or ops[k](candidate.__dict__[k], mfcsam[k]):
				continue
			else:
				fail = True
				break
